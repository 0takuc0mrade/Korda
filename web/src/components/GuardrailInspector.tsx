...
const MOCK_DATA: Record<string, any> = {
  "req_003": {
    id: "req_003",
    status: "INTERCEPTED",
    targetNode: "AUTH_API_V1",
    originalPrompt: `def authenticate_user(token):\n    # Using v1 auth endpoint as requested\n    response = requests.post("https://api.internal/v1/auth", data={"token": token})\n    return response.json()`,
    hardenedPrompt: `[KORDA GUARDRAIL]: Dependency AUTH_API_V1 is STALE.\nTarget alternative active node AUTH_API_V2 for code generation.\n\ndef authenticate_user(token):\n    # Using v2 auth endpoint as requested\n    response = requests.post("https://api.internal/v2/auth", data={"token": token})\n    return response.json()`,
    confidence: "98.3%",
  ...
  "req_005": {
    id: "req_005",
    status: "INTERCEPTED",
    targetNode: "AUTH_LOGIC_LEGACY",
    originalPrompt: `class AuthManager:\n    def __init__(self):\n        self.legacy_mode = True\n        self.salt = "hardcoded_salt_123"`,
    hardenedPrompt: `[KORDA GUARDRAIL]: Dependency AUTH_LOGIC_LEGACY is STALE.\nTarget alternative active node SECURE_VAULT for code generation.\n\nclass AuthManager:\n    def __init__(self):\n        self.legacy_mode = False\n        self.salt = vault.get_secret("auth_salt")`,
    confidence: "99.1%",
